...
import { initVideoMetadata as initVideoMetadataModule } from './video-metadata';
...
(async function () {
  ...
  // Bump to 7: add video_metadata store
  const DB_VERSION = 7;
  ...
  async function executeTransaction<T>(
    ...
    // Attempt to open DB and create transaction. If the requested object store doesn't exist
    // (NotFoundError), close the cached connection, reopen (to trigger onupgradeneeded if needed),
    // and retry once.
    ...
    while (true) {
      ...
      try {
        ...
        // Detect NotFoundError caused by missing object store and retry once after reopening DB
        const name = err?.name || '';
        const message = String(err || '');
        const isNotFound = name === 'NotFoundError' || /object stores? was not found/i.test(message) || /NotFoundError/.test(message);
        if (isNotFound && !triedReopen) {
          triedReopen = true;
          log(`IndexedDB store ${storeName} not found; attempting to create the store and retrying transaction`);
          try {
            // Close and clear cached connection so we can upgrade
            if (db) {
              try { db.close(); } catch {}
            }
            // Ensure the object store exists by forcing an upgrade that creates it
            await IDB.ensureObjectStoreExists(storeName);
          } catch (closeErr) {
            log('Failed to create missing object store during ensure step:', closeErr, 'warn');
            // ignore and let retry happen which will likely fail and be reported
          }
  ...
  async function saveToIndexedDB(videoId: string, data: TimestampRecord[]): Promise<void> {
    // Save to v2 store only using idb
    const db = await IDB.getDb();
    const tx = db.transaction(STORE_NAME_V2, 'readwrite');
    const v2Store: any = (tx as any).objectStore(STORE_NAME_V2);

    try {
      const existingRecords = await v2Store.index('video_id').getAll(IDBKeyRange.only(videoId));
      const newGuids = new Set(data.map(ts => ts.guid));

      // Delete removed timestamps
      for (const record of existingRecords as Array<{guid: string}>) {
      ...
      for (const ts of data) {
        v2Store.put({ guid: ts.guid, video_id: videoId, start: ts.start, comment: ts.comment });
      }
  ...
  async function saveSingleTimestampToIndexedDB(videoId: string, timestamp: TimestampRecord): Promise<void> {
    const db = await IDB.getDb();
    const tx = db.transaction(STORE_NAME_V2, 'readwrite');
    const v2Store: any = (tx as any).objectStore(STORE_NAME_V2);
    try {
      await v2Store.put({ guid: timestamp.guid, video_id: videoId, start: timestamp.start, comment: timestamp.comment });
      await (tx as any).done;
    } catch (err) {
      throw err ?? new Error('Failed to save single timestamp to IndexedDB');
    }
  }

  async function deleteSingleTimestampFromIndexedDB(videoId: string, guid: string): Promise<void> {
    ...
    const v2Store: any = (tx as any).objectStore(STORE_NAME_V2);
    try {
      await v2Store.delete(guid);
      await (tx as any).done;
    } catch (err) {
      throw err ?? new Error('Failed to delete single timestamp from IndexedDB');
    }
  }

  async function loadFromIndexedDB(videoId: string): Promise<TimestampRecord[] | null> {
    // Load from v2 store only using idb
    try {
      const db = await IDB.getDb();
      const tx = db.transaction(STORE_NAME_V2, 'readonly');
      const v2Store: any = (tx as any).objectStore(STORE_NAME_V2);
      const v2Records = (await v2Store.index('video_id').getAll(IDBKeyRange.only(videoId))) as Array<{guid: string; video_id: string; start: number; comment: string}>;
      if (v2Records.length > 0) {
        const timestamps = v2Records.map(r => ({ guid: r.guid, start: r.start, comment: r.comment })).sort((a, b) => a.start - b.start);
        return timestamps;
      }
      return null;
    } catch (err) {
  ...
  async function removeFromIndexedDB(videoId: string): Promise<void> {
    // Remove all timestamps for a video from v2 store using idb
    try {
      const db = await IDB.getDb();
      const tx = db.transaction(STORE_NAME_V2, 'readwrite');
      const v2Store: any = (tx as any).objectStore(STORE_NAME_V2);
      const records = (await v2Store.index('video_id').getAll(IDBKeyRange.only(videoId))) as Array<{guid: string}>;
      for (const record of records) {
        await v2Store.delete(record.guid);
      }
      await (tx as any).done;
    } catch (err) {
  ...
  function saveGlobalSettings(key: string, value: unknown): Promise<void> {
    return IDB.put(SETTINGS_STORE_NAME, { key, value }).catch(err => {
  ...
  function loadGlobalSettings(key: string): Promise<unknown> {
    return IDB.get(SETTINGS_STORE_NAME, key).then(result => {
  ...
  // Initialize video metadata via the video-metadata module

  async function initVideoMetadata(): Promise<void> {
    try {
      await initVideoMetadataModule({
        loadGlobalSettings,
        saveGlobalSettings,
        executeTransaction,
      ...
      log('Failed to initialize video metadata:', err, 'warn');
    }
  }

  // Kick off video metadata initialization after DB helpers are available.
  // Run asynchronously so startup isn't blocked; log failures.
  initVideoMetadata().catch(err => log('initVideoMetadata failed:', err, 'warn'));
